...
const validateuser = async () =>{
    ...
    <p onclick ='showData(3)'><i class="bi bi-journal-album"></i>My Album</p>
    <p onclick ='showData(4)'><i class="bi bi-person"></i>My Profile</p>
    <p onclick ='showData(5)'><i class="bi bi-file-check"></i>My Todos</p>
...
const getPosts = async () => {
    ...
    json.map((element) => {
        if(userId==element.userId){str +=`<p><b>User:</b>${element.userId}</p>
        <p><b>Title:</b>${element.title}</p>
        <p><b>Body:</b>${element.body}</p>
        <p onclick = getComments(${element.id})>View comments</p>
        <hr>`
...
const getProfile = async () => {
    ...
    json.map((element) => {
        if(userId==element.id){str +=`<p><b>Id:</b>${element.id}</p>
